...
```
 ...
 |                                                                    |
 |   [prometheus] [loki + promtail] [grafana]   (chỉ qua SSH tunnel)  |
 +--------------------------------------------------------------------+
```

**Text alternative**: Internet chỉ vào được cổng 80/443 của Nginx. Nginx nằm trên mạng `edge` cùng frontend và backend. Backend và worker nằm trên mạng `internal` cùng PostgreSQL, Redis và RabbitMQ; ba datastore này không có cổng public. Worker đọc RabbitMQ và gửi mail tới SMTP bên ngoài. Prometheus, Loki và Grafana chạy trên cùng VPS, chỉ truy cập qua SSH tunnel.
...
- Có thể bỏ Prometheus/Loki/Grafana để nhẹ máy.
...
4. Lỗi → queue retry; hết lượt → DLQ + cảnh báo Grafana.
...
4. **Dữ liệu cũ không khôi phục được** vì không có backup (ngoại lệ REL-008). Tài khoản phải được admin tạo/nhập lại.
