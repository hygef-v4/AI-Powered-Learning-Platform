...
- Mỗi email: `INCR email:daily-count:{yyyyMMdd}`; vượt 300 → `DECR`, chuyển mọi mục còn lại sang `DEFERRED`, `scheduled_for = 00:05 ngày sau`.
...
- Job `DEADLINE_REMINDER {publicationId, expectedClosesAt}`; chạy thì so với `closesAt` hiện tại (U08), khác → không nhắc và tạo job mới theo hạn mới nếu còn ở tương lai (NFR-U16-11).
