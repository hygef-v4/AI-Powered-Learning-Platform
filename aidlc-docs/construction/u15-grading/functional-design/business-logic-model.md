...
## F1 - Nhận bài nộp
1. `u11.submission.submitted` → tạo `Grade` `ATTEMPT` `PENDING`, `maxScore` = tổng điểm bài.
2. Trắc nghiệm → tự chấm ngay (BR-U15-10), áp BR-U15-12.
3. `u14.group.submitted` → `Grade` `GROUP_DOCUMENT` + `MEMBER_CONTRIBUTION`/`MEMBER_FINAL` cho mỗi thành viên, `PENDING` (bản nộp mới thay bản cũ: điểm chưa chốt gắn sang bản mới).
4. `u13.code.graded` → cập nhật `autoScore`/`items`, `DRAFT` (BR-U15-11).
