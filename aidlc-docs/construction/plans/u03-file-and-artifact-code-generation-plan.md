...
- **Story/UC**: không có trực tiếp (hạ tầng dùng chung, kiểm ở gate G1). Phục vụ U01 (avatar), U05, U09, U11, U13, U14, U16.
...
| `JobPort`, `JobHandler` (job `U03_DRIVE_DELETE`, `U03_DRIVE_CLEANUP`) | U02 | Dùng U02 thật nếu đã code; chưa có thì chờ U02 Bước 4-7 (U03 mở sau U01/U02 theo `unit-of-work.md`) |
| `AuditPort` | U02 | Như trên |

### Dữ liệu U03 sở hữu

PostgreSQL `artifacts`; Redis `u03:dl:*`; thư mục trên Google Shared Drive; queue `jobs.u03.drive-delete`, `jobs.u03.drive-cleanup`.
...
- [ ] **Bước 3** - Domain: `Artifact` với trạng thái `ACTIVE`/`BLOCKED`/`DELETED` và chuyển trạng thái hợp lệ; `ArtifactPurpose` (`AVATAR`, `MATERIAL`, `DRAWIO_FULL`, `SUBMISSION_FILE`, mục đích dẫn xuất); `PurposePolicy` (allowlist loại file, vai trò được upload); `FileNameSanitizer` (BR-U03-03, 04, 08).
- [ ] **Bước 4** - Port: `ArtifactPort` (`store`, `storeDerived`, `attach`, `issueDownloadToken`, `open`, `deleteDerived`), `StoragePort`.
- [ ] **Bước 5** - `ContentInspector`: Tika trên 8 KB đầu, so allowlist; parser XML an toàn (tắt DOCTYPE, external entity, XInclude), gốc `mxfile`/`mxGraphModel`; lỗi trả thông điệp chung; ZIP chỉ kiểm magic bytes (BR-U03-03, 10-12, P3).
...
- [ ] **Bước 7** - `ArtifactService`: `attach` (một lần, đúng người tải lên hoặc hệ thống), `storeDerived` (file gốc `ACTIVE`, có `sourceArtifactId`), `deleteDerived` (chỉ file dẫn xuất, đánh dấu `DELETED` rồi tạo job `U03_DRIVE_DELETE`) (BR-U03-13, 30-33, P7).
- [ ] **Bước 8** - `DownloadTokenService`: token 32 byte base64url, Redis `u03:dl:{sha256}` TTL 5 phút gắn `accountId`; từ chối `BLOCKED`/`DELETED`; kiểm chủ token khi dùng, sai thì `404` và audit (BR-U03-20, 21, 25, 40, P4).
- [ ] **Bước 9** - `DriveJobHandler` trong worker cho `U03_DRIVE_DELETE` và `U03_DRIVE_CLEANUP`; đăng ký với `JobHandlerRegistry` của U02 và khai báo queue `jobs.u03.*`.
- [ ] **Bước 10** - Audit các sự kiện của BR-U03-40; không log `providerFileId`, key, token.
- [ ] **Bước 11** - Unit test cho mọi `BR-U03-xx`, gồm XML có DOCTYPE/XXE, file đổi đuôi, file > 50 MB, upload thứ 6 bị `503`, token dùng sai người, dọn Drive khi INSERT lỗi.
- [ ] **Bước 12** - Tóm tắt: `aidlc-docs/construction/u03-file-and-artifact/code/business-logic-summary.md`.

### Nhóm C - Dữ liệu và lưu trữ

- [ ] **Bước 13** - Flyway `V20260925_1000__u03_artifacts.sql`: bảng `artifacts` theo `domain-entities.md`, index `(scope_type, scope_id)`, `source_artifact_id`, unique `provider_file_id`.
...
- [ ] **Bước 17** - `DriveHealthIndicator`: `drives.get` cache 60 s, Drive lỗi không làm backend `DOWN` (P8).
- [ ] **Bước 18** - Integration test Testcontainers (PostgreSQL, Redis) với `LocalFolderStorageAdapter`: upload → attach → cấp token → tải; token hết hạn; `deleteDerived` tạo job; adapter Drive test bằng mock HTTP (lỗi 429, abuse).
...
- [ ] **Bước 21** - `FileUploadController`, `DownloadController` (`StreamingResponseBody`, bộ đệm 64 KB, header `Content-Type`, `Content-Disposition` RFC 5987, `nosniff`, `Cache-Control: private, no-store`; `BLOCKED` → `410`) (BR-U03-22, 23, P2).
- [ ] **Bước 22** - Test MockMvc: không trả `providerFileId`, header đúng, sai `purpose`/vai trò bị từ chối, file quá lớn `413`.
...
- [ ] **Bước 25** - Test frontend: chặn file > 50 MB phía client, hủy upload, hiển thị lỗi backend.
...
- Tạo bản XML rút gọn cho AI (U13).
