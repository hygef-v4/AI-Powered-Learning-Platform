...
| Phát sự kiện | Sự kiện sau commit → RabbitMQ fanout `u14.realtime` → mỗi backend đẩy tới emitter của mình (worker cũng phát được, ví dụ tự nộp) | Không phụ thuộc số instance |
