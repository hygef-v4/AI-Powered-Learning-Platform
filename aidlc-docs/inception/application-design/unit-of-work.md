...
- Mỗi unit sở hữu dữ liệu và quy tắc nghiệp vụ của mình; unit khác gọi public contract, không đọc bảng/repository trực tiếp. Năm bảng dùng chung (`accounts`, `app_settings`, `assignments`, `publications`, `student_groups`) cho unit khác thêm cột của mình và ghi qua port của unit chủ (xem `component-dependency.md` mục 4a).
...
| U09 | Question Type Authoring | Cấu hình quiz/essay/tài liệu (DOCUMENT), mô hình tài liệu có sơ đồ Draw.io nhúng, nhập khung từ DOCX và preview DOCX của người học trong lượt DOCUMENT, xuất bài ra DOCX, quy tắc kiểm XML. Không có đề chung cấp môn. Ghi cấu hình loại bài và khung vào cột của `assignments` (thêm cột bằng migration của U09, ghi qua `AssignmentExtensionPort`) | Không sở hữu bank item, publication transaction, attempt hay sandbox |
| U10 | Template, Copy & Simulation | Template môn, copy assignment/rubric giữa lớp có lineage, simulation policy, diff giữa các version (nhân bản và ngưng giao ở U08). Sở hữu bảng `template_releases`; ghi lineage vào cột của `assignments` và chính sách thi thử vào cột của `publications` qua `AssignmentExtensionPort` | Không copy lớp/khóa học, publication, attempt, submission hay grade |
...
| U16 | Reporting & Notification | Thông báo hệ thống trong app (SSE), email có trần và tùy chọn tắt, nhắc hạn 24 giờ tự động, tiến độ nộp bài; dashboard điểm cá nhân và xuất bảng điểm theo yêu cầu | Không ghi ngược transaction nguồn hoặc rollback nghiệp vụ; không làm báo cáo phân tích nâng cao hoặc đối sánh điểm AI với điểm giảng viên |

### Quyết định riêng cho phần Learning Access của U04

Catalog hiện hành có `US-LRN-001` cho quyền truy cập lớp. `UC-LRN-01`, `UC-LRN-02` và `UC-CNT-04` hỗ trợ tổng quan lớp/truy cập nội dung; không theo dõi tiến độ hoàn thành bài học. Các UC/story tiến độ đã được xóa khỏi danh mục. Không có requirement/story về learning path, vì vậy U04 không tạo lộ trình, prerequisite hay completion model. Dashboard kết quả cá nhân thuộc U16.
